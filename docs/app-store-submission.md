...
| 10 | **Paid Applications agreement Active** | **Done 2026-08-22** | `Active`, Aug 17 2026 – Apr 23 2027. Banking `Active` (Revolut Bank UAB, Netherlands, EUR). Both tax forms `Active`, submitted Aug 22: the Certificate of Foreign Status, and W-8BEN claiming the US–Latvia treaty, **Article 12(2)(b), 10%**, on income from the sale of applications — 12(2)(a)'s 5% is equipment only. If the tax residence on file ever moves to the Netherlands, that claim becomes Article 13 at 0% and needs a new form within 30 days. The €5 tier is now settable. | App Store Connect → Business |
| 11 | **DSA trader declaration** | **Submitted, `In Review`** | Filed 2026-08-16 for 27 countries. It will display Latvia, because the App Store account is `Individual · Riga · Latvia` and the country cannot be self-service changed; the legal pages say Netherlands. That inconsistency resolves on the app transfer to a Dutch account, which requires a released version to be possible at all. | App Store Connect → Business → Compliance |
| 13 | **Updated Apple Developer Program License Agreement accepted** | **Open — Account Holder only** | A banner on Business → Agreements says the agreement was updated and that until the Account Holder reviews and accepts it, no new app can be submitted and no existing app updated. Independent of Paid Apps, and it blocks submission on its own. | App Store Connect → Business, banner link → developer.apple.com account |
...
- Reviewer code: **not written down here — this repository is public.** A code printed in a public
  file can have its 50 uses burned by anyone, and the failure mode is App Review signing in and
  finding an empty library. The live one was minted 2026-08-21, is valid to 2026-12-19, and lives in
  the App Store Connect review notes and `manage_invites.py list`. `STASH-JJGG-7HW9`, published in
  this file before that was noticed, was revoked on 2026-08-21.
- Keep the code out of commits, screenshots and the demo video. If it ever does land in one, mint a
  replacement rather than deciding it probably went unnoticed.
...
2. IMPORTANT, so you do not land in an empty app: on the sign-in screen, tap "Have a code?"
   below the button and enter
       [[REVIEWER_CODE — paste the live one straight into App Store Connect, not into this file]]
