...
| 10 | **Paid Applications agreement Active** | **In flight — owner only** | Checked 2026-08-21: the agreement is signed and sitting at `Pending User Info` (Aug 17 2026 – Apr 23 2027), banking is submitted (Revolut Bank UAB, Netherlands, EUR) and `Processing` with edits locked for ~24h, and what holds it back is two tax forms at `Missing Tax Info`: **U.S. Certificate of Foreign Status of Beneficial Owner** and **U.S. Form W-8BEN**. Both are prefilled from the account and need a TIN and a signature. Submit is final — the form says so. | App Store Connect → Business |
